...
if __name__ == '__main__':
    ...
    save_name = f"{current_dir}/genaudios/Make-an-Audio-2/"
    ...
    for k in range(runs):
        ...
        tracker = EmissionsTracker(project_name=f"Make-an-Audio-2-inference_batch10", tracking_mode="process", gpu_ids="1",
                           output_dir=f"{parent_dir}/results/batch_size/Make-an-Audio-2",
                           output_file=f"Make-an-Audio-2-emissions-batch10-run{k+1}.csv", allow_multiple_runs=True)
        ...
        try:
            ...
            wav_list = gen_wav(sampler,vocoder,prompt=prompto,struct_prompt=struct,ddim_steps=100,scale=3,duration=10,n_samples=10)
    ...
    for k in range(runs):
        file_counter = 0
        for i in range(2):
            tracker = EmissionsTracker(project_name=f"Make-an-Audio-2-inference_batch5", tracking_mode="process", gpu_ids="1",
                            output_dir=f"{parent_dir}/results/batch_size/Make-an-Audio-2/batch5",
                            output_file=f"Make-an-Audio-2-emissions-batch5-n{i}-run{k+1}.csv", allow_multiple_runs=True)
            tracker.start_task(f"Inference emissions with 5 waveforms per prompt")

            try:

                print("Generating audios, it may takes a long time depending on your gpu performance")
                wav_list = gen_wav(sampler,vocoder,prompt=prompto,struct_prompt=struct,ddim_steps=100,scale=3,duration=10,n_samples=5)
                model_emissions = tracker.stop_task()
    ...
    for k in range(runs):
        file_counter = 0
        for i in range(5):
            tracker = EmissionsTracker(project_name=f"Make-an-Audio-2-inference_batch2", tracking_mode="process", gpu_ids="1",
                            output_dir=f"{parent_dir}/results/batch_size/Make-an-Audio-2/batch2",
                            output_file=f"Make-an-Audio-2-emissions-batch2-n{i}-run{k+1}.csv", allow_multiple_runs=True)
            tracker.start_task(f"Inference emissions with 2 waveforms per prompt")

            try:

                print("Generating audios, it may takes a long time depending on your gpu performance")
                wav_list = gen_wav(sampler,vocoder,prompt=prompto,struct_prompt=struct,ddim_steps=100,scale=3,duration=10,n_samples=2)
                model_emissions = tracker.stop_task()
    ...
    for k in range(runs):
        for i in range(10):
            tracker = EmissionsTracker(project_name=f"Make-an-Audio-2-inference_batch1", tracking_mode="process", gpu_ids="1",
                            output_dir=f"{parent_dir}/results/batch_size/Make-an-Audio-2/batch1",
                            output_file=f"Make-an-Audio-2-emissions-batch1-n{i}-run{k+1}.csv", allow_multiple_runs=True)
            tracker.start_task(f"Inference emissions with 1 waveforms per prompt")

            try:

                print("Generating audios, it may takes a long time depending on your gpu performance")
                wav_list = gen_wav(sampler,vocoder,prompt=prompto,struct_prompt=struct,ddim_steps=100,scale=3,duration=10,n_samples=1)
                model_emissions = tracker.stop_task()
...
emissions_base = glob.glob(f'{parent_dir}/results/batch_size/Make-an-Audio-2/emissions_base_*')
for f in emissions_base:
    os.remove(f)
emissions_base = glob.glob(f'{parent_dir}/results/batch_size/Make-an-Audio-2/batch5/emissions_base_*')
for f in emissions_base:
    os.remove(f)
emissions_base = glob.glob(f'{parent_dir}/results/batch_size/Make-an-Audio-2/batch2/emissions_base_*')
for f in emissions_base:
    os.remove(f)
emissions_base = glob.glob(f'{parent_dir}/results/batch_size/Make-an-Audio-2/batch1/emissions_base_*')
for f in emissions_base:
    os.remove(f)
