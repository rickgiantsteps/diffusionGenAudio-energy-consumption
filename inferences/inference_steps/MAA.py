...
for k in range(runs):
    for x in n_step:
        ...
        tracker = EmissionsTracker(project_name=f"Make-an-Audio-inference_{x}-steps", tracking_mode="process", gpu_ids="1",
                                   output_dir=f'{parent_dir}/results/inference_steps/Make-an-Audio',
                                   output_file=f"Make-an-Audio-emissions-run{k + 1}.csv", allow_multiple_runs=True)
        ...
        try:
            ...
            scl = 3; length = 10; samples = 1; save_name = "Make-an-Audio"
            wav_list = gen_wav(sampler,vocoder,prompt=prompto,ddim_steps=x,scale=scl,duration=length,n_samples=samples)
        ...
        for idx,wav in enumerate(wav_list):
            soundfile.write(f'{current_dir}/genaudios/Make-an-Audio/{save_name}_{x}-steps.wav',wav,samplerate=SAMPLE_RATE)
            print(f"audios are saved in {current_dir}/genaudios/Make-an-Audio/{save_name}_{x}-steps.wav")

emissions_base = glob.glob(f'{parent_dir}/results/inference_steps/Make-an-Audio/emissions_base_*')
